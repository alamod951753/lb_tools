...
namespace ToolsDev.Repository.Extensions
{
    public static class Extensions
    {
            ...
            string typeName, IEnumerable<string> orderedColumnNames = null)
        {
            ...
            else
            {
                ...
                {
                    var property = readableProperties.Single(s => s.Name.Equals(name)).PropertyType;
                    if (property.Name.Contains("Nullable"))
                    {
                        dataTable.Columns.Add(name, Nullable.GetUnderlyingType(property) ?? property);
                    }
                    else
                    {
